...
const HomePage = () => {
  ...
  const columns = [
    ...
    { title: "Description", dataIndex: "description", key: "description" },
    {
      ...
      render: () => (
        <span>
          <Button type="link" style={{ padding: 0 , marginRight: 8 }}>
            Edit
          </Button>
          <Button type="link" style={{ padding: 0, color: "red" }}>
            Delete
          </Button>
        </span>
      ),
  ...
  const getAllTransaction = useCallback(async () => {
    try {
      ...
      if (frequency === "custom" && selectedDate.length === 2) {
        ...
        payload.frequency = parseInt(frequency, 10);
      }
    ...
    <Layout>
      ...
      <div className="filters">
        <div>
          ...
          {frequency === "custom" && (
            <RangePicker value={selectedDate} onChange={(dates) => setSelectedDate(dates)} />
          )}
        </div>
        <div>
          <h6>Select Type</h6>
  ...
  />
      ...
      <div className="content">
        {viewData === 'table' ? (
          <Table columns={columns} dataSource={allTransaction} rowKey="_id" pagination={{ pageSize: 5 }} />
        ) : (
